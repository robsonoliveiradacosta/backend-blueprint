...
- **Type** — exactly one of the following. Nothing outside this list:
  ...
  into the list: `build` → `chore`, `revert` → see below. `wip` is not a commit that reaches a shared branch.
...
- **A revert is a commit like any other.** `git revert` proposes `Revert "<old subject>"`, which fails the standard —
  rewrite it. The type is the effect the revert has (`fix` when it undoes a broken change, `chore` when it undoes an
  experiment), and the reverted commit goes in a footer: `Reverts: 8ba09f9`.
...
| `build(deps): bump the framework` | `build` is not one of the nine — this is `chore` |
| `feat(api)!: rename the field` + `Breaking change: …` | the footer token must be uppercase `BREAKING CHANGE:` |
| `Revert "feat(user): add endpoint"` | git's default revert message — rewrite it with a type and `Reverts:` |

## Before committing

- [ ] The message describes what is staged, not what you intended to do
- [ ] Type is one of the nine, and honest about the effect of the diff
